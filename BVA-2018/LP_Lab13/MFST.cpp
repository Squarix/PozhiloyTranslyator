...
namespace MFST
{
	...
	bool Mfst::start(Log::LOG log)
	{
		...
		case LENTA_END:
			...
			*log.stream << std::setfill('-') << std::setw(100) << ' ' << std::setfill(' ') << std::endl;
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: Синтаксияеский разбор выполнен успешно", 0, lenta_size);
	...
	char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "Ошибка %d: строка %d, %s", err.id, lexTable.table[lpos].sn, err.message);
			throw ERROR_THROW_IN(err.id, lexTable.table[lpos].sn, 0);
			rc = buf;
		}
		return rc;
	}
